...
public:
	...
	*/
	...
	{
		Quaternion q(0,
			vector.GetVector3D().x * scale,
			vector.GetVector3D().y * scale,
			vector.GetVector3D().z * scale);
		q *= *this;
		r += q.r * 0.5f;
		i += q.i * 0.5f;
		j += q.j * 0.5f;
	...
	{
		Quaternion q(0, vector.GetVector3D().x, vector.GetVector3D().y, vector.GetVector3D().z);
		(*this) *= q;
	}
};
	...
	const Quaternion &orientation)
{
	...
	transformMatrix.r[3] = XMVectorSetX(transformMatrix.r[3], position.GetVector3D().x);
	transformMatrix.r[3] = XMVectorSetY(transformMatrix.r[3], position.GetVector3D().y);
	transformMatrix.r[3] = XMVectorSetZ(transformMatrix.r[3], position.GetVector3D().z);
